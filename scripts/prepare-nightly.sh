...
while IFS= read -r immutable_tag; do
  ...
  [ "$release_count" -eq 1 ] || {
    peeled="$(git rev-parse "refs/tags/$immutable_tag^{}")"
    if [ "$release_count" -eq 0 ]; then
      echo "prepare-nightly: outstanding nightly transaction $immutable_tag at $peeled does not have exactly one release object; dispatch Nightly Recovery recover-orphan-tag with release_id=absent, expected SHA $peeled, and confirmation equal to the tag" >&2
    else
      echo "prepare-nightly: outstanding nightly transaction $immutable_tag at $peeled does not have exactly one release object; use recovery" >&2
    fi
    exit 1
  }
  jq -e --arg tag "$immutable_tag" \
    ...
    <<<"$nightly_releases" >/dev/null || {
      echo "prepare-nightly: outstanding nightly transaction $immutable_tag is not completely published; dispatch Nightly Recovery recover-orphan-tag or resume-draft" >&2
      exit 1
    }
