...
async fn register_provider_enabled_route_seeds(
    ...
    // Keep the provenance-safe fallback cache in lockstep with the routes
    // published while enabling this provider. A following seed read can fail
    // before discovery refreshes it again.
    let mut seeds = state.model_route_seeds.write().await;
