(() => {
  ...
  function integerTicks(max, target = 4) {
    ...
    const raw = value / Math.max(1, target);
  ...
  // Size the canvas buffer to the container's CSS size times the device pixel
  // ratio. The old fixed 1200px buffer was scaled down by `width: 100%`, which
  // shrank every chart font below its intended size. With the buffer matching
  // the display size, 10px chart text renders at a true 10px (crisper on HiDPI).
  function fitCanvas(canvas, cssHeight) {
    const dpr = window.devicePixelRatio || 1;
    const cssW = canvas.clientWidth || canvas.width;
    const w = Math.max(100, Math.round(cssW * dpr));
    const h = Math.max(80, Math.round((cssHeight || 220) * dpr));
  ...
  function lineTooltipHtml(point, range, colors) {
    return (
      `<div class="tt-title">${esc(formatBucketLabel(point.ts, range))}</div>` +
      tooltipRowsHtml([
  ...
  function barTooltipHtml(row, range, colors) {
    return (
      `<div class="tt-title">${esc(row.key || formatBucketLabel(row.ts, range))}</div>` +
      tooltipRowsHtml([
  ...
  function tooltipSummary(point, range) {
    const label = formatBucketLabel(point.ts, range);
  ...
  function announceChartData(text) {
    const live = document.getElementById("chart-live");
    if (live) live.textContent = text;
  }

  function chartToClient(canvas, bufferX, bufferY) {
  ...
  function showLineTooltipFor(canvas, state, idx) {
    ...
    showChartTooltip(canvas, pos.x, pos.y, lineTooltipHtml(point, state.range, chartColors()));
    announceChartData(tooltipSummary(point, state.range));
  }

  function showBarTooltipFor(canvas, state, idx) {
    const row = state.rows[idx];
    const g = state.geometry;
    const pos = chartToClient(canvas, g.xAt(idx) + g.barW / 2, g.yAt(row.total_tokens || 0));
    showChartTooltip(canvas, pos.x, pos.y, barTooltipHtml(row, state.range, chartColors()));
    announceChartData(tooltipSummary(row, state.range));
  }
  ...
  function resolveLineIdx(state) {
    if (state.hoverTs == null || !state.geometry) return -1;
    return state.series.findIndex((p) => p.ts === state.hoverTs);
  }

  function resolveBarIdx(state) {
    if (state.hoverTs == null || !state.geometry) return -1;
    return state.rows.findIndex((r) => r.ts === state.hoverTs);
  }
  ...
  function attachChartHover(canvas) {
    ...
    canvas.addEventListener("mousemove", (event) => {
      ...
      if (!state) return;
      canvas.__mouse = { x: event.clientX, y: event.clientY };
    ...
    canvas.addEventListener("focus", () => {
      const state = canvas.__chart;
      const points = state ? chartPoints(state) : [];
      if (!points.length) return;
      const cur = state.kind === "line" ? resolveLineIdx(state) : resolveBarIdx(state);
      const next = cur < 0 ? points.length - 1 : cur;
      state.hoverTs = points[next].ts;
      if (state.kind === "line") drawLineChart(canvas, state.series, state.range);
      else drawBarChart(canvas, state.rows, state.range);
    });
    ...
    canvas.addEventListener("keydown", (event) => {
      ...
      const next = Math.max(0, Math.min(points.length - 1, (cur < 0 ? points.length - 1 : cur) + delta));
      state.hoverTs = points[next].ts;
      if (state.kind === "line") drawLineChart(canvas, state.series, state.range);
      else drawBarChart(canvas, state.rows, state.range);
    });
  ...
  function drawLineChart(canvas, series, range) {
    ...
    hideChartTooltip(canvas);
    if (!series.length) {
      canvas.__chart = { kind: "line", series: [], range, geometry: null, hoverTs: null };
      return;
    }
    const padT = 30;
    const padB = 26;
    ...
    canvas.__chart = {
      ...
      series,
      range,
      geometry: { xAt, yTokens, yPrompts, ySessions, padT, plotH, padL, padR, axisX, tokens, prompts, sessions },
      hoverTs: prev && prev.kind === "line" ? prev.hoverTs : null,
    };
    ...
    if (idx >= 0) {
      ...
      if (canvas.__mouse) {
        showChartTooltip(canvas, canvas.__mouse.x, canvas.__mouse.y, lineTooltipHtml(series[idx], range, colors));
      } else {
  ...
  function handleLineChartHover(canvas, event, state) {
    const { series, range, geometry } = state;
    const rect = canvas.getBoundingClientRect();
    ...
    series.forEach((point, i) => {
      const dist = Math.abs(geometry.xAt(point.ts) - mx);
      if (dist < bestDist) {
        bestDist = dist;
        best = i;
      }
    });
    const threshold = Math.max(10, 14 * scaleX);
    ...
    if (state.hoverTs !== series[best].ts) {
      state.hoverTs = series[best].ts;
      drawLineChart(canvas, series, range);
    } else {
      showChartTooltip(canvas, event.clientX, event.clientY, lineTooltipHtml(series[best], range, chartColors()));
    }
  }
  ...
  function drawBarChart(canvas, rows, range) {
    ...
    hideChartTooltip(canvas);
    if (!rows.length) {
      canvas.__chart = { kind: "bar", rows: [], range, geometry: null, hoverTs: null };
      return;
    }
    const padT = 30;
    const padB = 26;
    ...
    const barGap = rows.length > 60 ? 1 : 6;
    const barW = Math.max(0, (plotW - barGap * (rows.length - 1)) / rows.length);
    const xAt = (i) => padL + i * (barW + barGap);
    ...
    canvas.__chart = {
      ...
      rows,
      range,
      geometry: { xAt, yAt, barW, plotH, padT, padL, max },
      hoverTs: prev && prev.kind === "bar" ? prev.hoverTs : null,
    };
    ...
    rows.forEach((r, i) => {
      ...
      if (rows.length <= 40 || i % Math.ceil(rows.length / 40) === 0 || i === rows.length - 1) {
        ...
        ctx.textBaseline = "top";
        ctx.fillStyle = colors.muted;
        ctx.fillText(String(r.key || "").slice(0, 8), x + barW / 2, h - padB + 4);
      }
    });
    ...
    if (hidx >= 0) {
      ...
      if (canvas.__mouse) {
        showChartTooltip(canvas, canvas.__mouse.x, canvas.__mouse.y, barTooltipHtml(rows[hidx], range, colors));
      } else {
        showBarTooltipFor(canvas, canvas.__chart, hidx);
      }
    }
  }

  function handleBarChartHover(canvas, event, state) {
    const { rows, range, geometry } = state;
    const rect = canvas.getBoundingClientRect();
    const scaleX = canvas.width / rect.width;
    const mx = (event.clientX - rect.left) * scaleX;
    let idx = -1;
    for (let i = 0; i < rows.length; i++) {
      const x0 = geometry.xAt(i);
      if (mx >= x0 && mx <= x0 + geometry.barW) {
        idx = i;
        break;
      }
    }
    ...
    if (state.hoverTs !== rows[idx].ts) {
      state.hoverTs = rows[idx].ts;
      drawBarChart(canvas, rows, range);
    } else {
      showChartTooltip(canvas, event.clientX, event.clientY, barTooltipHtml(rows[idx], range, chartColors()));
    }
  }
